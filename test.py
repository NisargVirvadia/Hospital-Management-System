...
def display():
        ...
        class Application:
            def __init__(self, master):
                #hi=Label(win3,text="HELLO")
                #hi.grid(row=1,column=1)
                self.master = master

                # creating the frames in the master
                self.left = Frame(win3, width=800, height=720, bg='lightgreen')
                self.left.pack(side=LEFT)
            
                self.right = Frame(master, width=600, height=720, bg='steelblue')
                self.right.pack(side=RIGHT)

        # labels for the window
                heading = Label(win3, text="ABC Hospital Appointments", font=('arial 40 bold'), fg='black', bg='lightgreen')
...
add = Button(root,text="ADD APPOINTMENT", width=20, height=2, bg='steelblue',command=addapp)
add.place(x=300, y=340)

submit = Button(root,text="UPDATE APPOINTMENT", width=20, height=2, bg='steelblue',command=update)
submit.place(x=500, y=340)

submit = Button(root,text="DISPLAY APPOINTMENT", width=20, height=2, bg='steelblue',command=display)
submit.place(x=700, y=340)
